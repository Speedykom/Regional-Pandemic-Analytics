    ...
    payload = {
      ...
      "spec": {
        "ioConfig": {
          ...
          "inputSource": {
            "type": "local",
            "baseDir": '/opt/shared/test-pipeline.parquet',
            "filter": "*.parquet"
          },
          "inputFormat": {
            "type": "parquet"
...
with DAG('covid-18-process', default_args=default_args, schedule_interval='@daily', catchup=False, is_paused_upon_creation=False) as dag:
    ...
    hop = DockerOperator(
        task_id='hop_task',
        image='apache/hop',
        container_name='covid-18-process',
        api_version='auto',
        auto_remove=True,
        host_tmp_dir='/files',
        mount_tmp_dir=False,
        ...
        environment={
            'HOP_LOG_LEVEL': 'Basic',
            'HOP_FILE_PATH': '${PROJECT_HOME}test-pipeline.hpl',
