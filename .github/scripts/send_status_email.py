...
def generate_email_content(services, project_name, subject_prefix=None, report_label="server"):
    ...
    <body>
        <p>Hello,</p>
        <p>This is your automated {report_label} status report generated by {project_name} Server Monitor CI/CD.<br>
        Timestamp: {timestamp}</p>
    """.format(project_name=project_name, timestamp=timestamp, report_label=report_label)
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python send_status_email.py <status_file> <project_name> [subject_prefix] [report_label]")
        sys.exit(1)
    
    status_file = sys.argv[1]
    project_name = sys.argv[2]
    subject_prefix = sys.argv[3] if len(sys.argv) > 3 else None
    report_label = sys.argv[4] if len(sys.argv) > 4 else "server"
    ...
    try:
        ...
        subject, html = generate_email_content(services, project_name, subject_prefix, report_label)
