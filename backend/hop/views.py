...
class GetSingleHopAPIView(APIView):
    ...
    def get(self, request, filename: str):
        ...
        if result:
            ...
            return Response({'status': 'error', "message": "No match found! No filename match: {}".format(filename)},
                            status=404)
        ...
        try:
            ...
            if (len(filename) != 0):
                ...
                if get_file_by_name(file_obj.name):
                    return Response({'status': 'error', "message": '{} already exists'.format(file_obj.name)},
                                    status=409)
                else:
                    # replace the file storage from filestorage to minio
                    upload_file_to_minio("hop-bucket", file_obj)
