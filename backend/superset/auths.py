...
def get_auth_token ():
    request_body = {
        'username': os.getenv('BACKEND_SUPERSET_USER'),
        'password': os.getenv('BACKEND_SUPERSET_PASS'),
        'provider':os.getenv('BACKEND_SUPERSET_PROVIDER'),
        'refresh': True,
    }
    
    url = os.getenv('BACKEND_SUPERSET_LOGIN')
    headers = {
