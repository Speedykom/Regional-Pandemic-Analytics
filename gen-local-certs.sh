...
if test -f "$CA"; then
    ...
    if command -v sudo 2&>1 >> /dev/null && command -v trust 2&>1 >> /dev/null; then
...
#Our services
service_table="frontend backend airflow druid hop keycloak minio console.minio superset openhim console.openhim coordinator"
