...
default_args = {
    'owner': 'airflow',
    'description': 'fetch-google-process',
    ...
    payload = {
      ...
      "spec": {
        "ioConfig": {
          ...
          "inputSource": {
            "type": "local",
            "baseDir": '/opt/shared/fetch-google-process.parquet',
            "filter": "*.parquet"
          },
          "inputFormat": {
            "type": "parquet"
        ...
        "dataSchema": {
          "dataSource": 'fetch google process',
          "timestampSpec": {
            "column": "Date",
            "format": "millis"
          },
...
with DAG('fetch-google-process', default_args=default_args, schedule_interval='@weekly', catchup=False, is_paused_upon_creation=False) as dag:
    ...
    hop = DockerOperator(
        task_id='hop_task',
        image='apache/hop',
        container_name='fetch-google-process',
        api_version='auto',
        auto_remove=True,
        host_tmp_dir='/files',
        mount_tmp_dir=False,
        ...
        environment={
            'HOP_LOG_LEVEL': 'Basic',
            'HOP_FILE_PATH': '${PROJECT_HOME}fetch-google-process.hpl',
