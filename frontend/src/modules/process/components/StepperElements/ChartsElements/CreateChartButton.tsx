...
export default function CreateChartButton({
    ...
    const [state, setState] = useState({ createChartUrl, hasDatasetUrl: !!createChartUrl });

    const { data, isSuccess, isLoading } = useGetDatasetInfoQuery(dagId, { skip: state.hasDatasetUrl, pollingInterval: 5_000 });
    if (isSuccess && !isLoading && data.dataset?.url) {
        setState({ ...state, createChartUrl: data.dataset.url, hasDatasetUrl: true });
    }
