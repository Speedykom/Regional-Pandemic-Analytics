...
default_args = {
    'owner': 'airflow',
    'description': 'fetch-json-api-process',
    ...
    payload = {
      ...
      "spec": {
        "ioConfig": {
          ...
          "inputSource": {
            "type": "local",
            "baseDir": '/opt/shared/fetch-json-api-process.parquet',
            "filter": "*.parquet"
          },
          "inputFormat": {
            "type": "parquet"
        ...
        "dataSchema": {
          "dataSource": 'fetch json api process',
          "timestampSpec": {
            "column": "Date",
            "format": "millis"
          },
...
with DAG('fetch-json-api-process', default_args=default_args, schedule_interval='@weekly', catchup=False, is_paused_upon_creation=False) as dag:
    ...
    hop = DockerOperator(
        task_id='hop_task',
        image='apache/hop',
        container_name='fetch-json-api-process',
        api_version='auto',
        auto_remove=True,
        host_tmp_dir='/files',
        mount_tmp_dir=False,
        ...
        environment={
            'HOP_LOG_LEVEL': 'Basic',
            'HOP_FILE_PATH': '${PROJECT_HOME}fetch-json-api-process.hpl',
