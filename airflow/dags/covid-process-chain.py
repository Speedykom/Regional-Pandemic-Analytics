...
default_args = {
    'owner': 'airflow',
    'description': 'covid-process-chain',
...
def ingest():
    ...
    payload = {
      ...
      "spec": {
        ...
        "dataSchema": {
          "dataSource": 'covid-process-chain',
          "timestampSpec": {
            "column": "Date",
            "format": "millis"
          },
...
with DAG('covid-process-chain', default_args=default_args, schedule_interval='@weekly', catchup=False, is_paused_upon_creation=False) as dag:
    ...
    hop = DockerOperator(
        task_id='hop_task',
        image='apache/hop',
        container_name='covid-process-chain',
        api_version='auto',
        auto_remove=True,
        host_tmp_dir='/files',
        mount_tmp_dir=False,
